...
const runReinit = (player: PlayerInstance, attempt: number): void => {
  const startedDefault = readDefaultId(player);  reinitPromise = player
    .reinitOutput()
    .then(() => {
      outputBroken = false;
...
export const requestReinit = (player: PlayerInstance): void => {
  // 流错误触发的重建
  if (pauseOnDeviceSwitch) player.pauseImmediately();  if (reinitPromise !== null || retryTimer !== null) {
...
const handleDeviceChange = (notifyListChange: boolean, defaultChanged: boolean): void => {
  ...
  try {
    ...
    if (shouldReinit || (outputBroken && currentDefault !== null && selectedDevice === null)) {      requestReinit(player);
    }
...
export const stopDeviceMonitoring = (): void => {
  ...
  outputBroken = false;
  pendingDefaultChange = false;  cancelPendingReinit();
};
