...
/**
 * 「我的歌单」按最近打开排序：最近打开的在前，未打开过的保持自然顺序
 * @param items - 歌单菜单项（key 为歌单路由路径，末段为歌单 ID）
 * @param pinnedKeys - 固定置顶的 key（不参与排序，永远在最前）
 */
const applyRecentFirst = <T extends { key: string }>(items: T[], pinnedKeys: string[] = []): T[] => {
  const lastOpened = appearance.sidebarPlaylistLastOpened;
  const pinned: T[] = [];
  const sortable = items.filter((item) => {
    if (pinnedKeys.includes(item.key)) {
      pinned.push(item);
      return false;
    }
    return true;
  ...
  const sorted = sortable
    .map((item, index) => ({
      item,
      index,
      at: lastOpened[item.key.split("/").pop() ?? ""] ?? 0,
    }))
...
const myPlaylistItems = computed<SMenuItem[]>(() => {
  ...
  // 「我喜欢的音乐」（网易云）已在上方 slice(1) 排除；本地爱心歌单固定置顶
  const ordered = applyRecentFirst(
    items,
    local ? [`/collection/local/playlist/${HEART_PLAYLIST_ID}`] : [],
  );
  const order = local
    ? appearance.sidebarPlaylistOrder.myLocal
    : appearance.sidebarPlaylistOrder.myOnline;
  return applySavedOrder(ordered, order).filter((item) => !hidden.has(item.key));});
