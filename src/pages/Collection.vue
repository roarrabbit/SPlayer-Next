...
import { HEART_PLAYLIST_ID } from "@/stores/playlist";
...
// 记录歌单最近打开时间，供侧边栏「我的歌单」按打开时间排序（爱心歌单固定置顶，不参与）
if (type === "playlist" && id !== HEART_PLAYLIST_ID) {
  useSettingsStore().appearance.sidebarPlaylistLastOpened[id] = Date.now();
}
...
const moreMenuItems = computed<DropdownMenuItem[]>(() => {
  ...
  // QQ/酷狗等无账号歌单体系的来源：提供整单导入本地歌单（在线 Track JSON 缓冲）
  const canImportToLocal = isOnline && source !== "netease";
  const list: DropdownMenuItem[] = [
    ...
    {
      key: "importToLocal",
      label: t("collection.importToLocal"),
      icon: IconLucideListMusic,
      show: canImportToLocal,
    },
    {
      key: "more",
...
const handleMoreMenu = (key: string) => {
  switch (key) {
    case "batchManage":
      ...
      break;
    case "importToLocal":
      if (collection.value?.tracks.length) openPicker(collection.value.tracks);
      break;
    case "edit":
      manage.openEdit();
      break;
