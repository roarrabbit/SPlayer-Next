...
| `@author`      |      | 作者                                                                                             |
| `@homepage`    |      | 主页 URL                                                                                         |
| `@grant`       |      | 控制插件声明权限：`network`（联网）/ `control`（控制播放器），逗号分隔；音源插件自动获 `network` |
...
- **控制插件**（`@type control`）要联网（如把当前曲目/歌词推给外部服务）必须声明 `@grant network`；要反向控制播放器必须声明 `@grant control`。
- 未授予对应权限时，`splayer.request` 以 `PLUGIN_PERMISSION_DENIED` 失败，`splayer.player.*` 调用被忽略。
